...
export default function ScenePage() {
  ...
  // Position guests in a semicircle arc around the table
  function getGuestPosition(index: number, total: number): { x: number, y: number } {
    if (total === 0) return { x: 50, y: 50 }

    const rank = index // 0 = highest score = closest
    const ratio = total === 1 ? 0 : rank / (total - 1) // 0 = closest, 1 = farthest

    // Arc from left to right, closer guests near center bottom
    const minRadius = 18  // closest (highest score)
    const maxRadius = 42  // farthest (lowest score)
    const radius = minRadius + ratio * (maxRadius - minRadius)

    // Spread angle: guests spread across 180deg arc below the table
    const minAngle = 200
    const maxAngle = 340
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--cream)', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400, padding: '24px' }}>
          <div style={{ background: '#fff', border: 'var(--border)', borderRadius: '20px', boxShadow: '6px 6px 0 #1a1a1a', width: '100%', maxWidth: '480px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px', display: 'flex', gap: '16px' }}>
              ...
              <div style={{ flex: 1, overflow: 'hidden' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                  {Array.from({ length: currentLayer?.count ?? 3 }).map((_, i) => {
            ...
            <div style={{ padding: '0 16px 16px', display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowHostEditor(false)}
                style={{ ...s.btn('var(--cream)'), flex: 1 }}>Cancel</button>
              <button onClick={saveHostConfig} disabled={saving}
                style={{ ...s.btn('var(--red)', '#fff'), flex: 1, opacity: saving ? 0.7 : 1 }}>
